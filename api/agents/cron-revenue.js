...
export default async function handler(req, res) {
  ...
  for (const { club_id } of rows) {
    try {
      const { managedSessionId } = await createAnalystSession(club_id, 'revenue_analyst');
      await sendMessage(managedSessionId, 'Run daily revenue analysis. Identify gaps vs forecast and conversion leaks.');

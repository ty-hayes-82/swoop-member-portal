...
export default async function handler(req, res) {
  ...
  for (const { club_id } of rows) {
    try {
      const { managedSessionId } = await createAnalystSession(club_id, 'service_recovery_analyst');
      await sendMessage(managedSessionId, 'Run service recovery scan. Check complaints older than 48h.');

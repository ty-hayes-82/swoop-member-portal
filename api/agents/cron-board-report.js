...
export default async function handler(req, res) {
  ...
  for (const { club_id } of rows) {
    try {
      const { managedSessionId } = await createAnalystSession(club_id, 'board_report_compiler');
      await sendMessage(managedSessionId, 'Compile monthly board report for prior month.');

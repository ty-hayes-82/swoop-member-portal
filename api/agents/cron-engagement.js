...
export default async function handler(req, res) {
  ...
  for (const { club_id } of rows) {
    try {
      const { managedSessionId } = await createAnalystSession(club_id, 'engagement_autopilot');
      await sendMessage(managedSessionId, 'Run engagement sweep. Identify dormant members for re-engagement.');

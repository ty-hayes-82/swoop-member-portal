...
export default async function handler(req, res) {
  ...
  for (const { club_id } of rows) {
    try {
      const { managedSessionId } = await createAnalystSession(club_id, 'labor_optimizer');
      await sendMessage(managedSessionId, "Run labor optimization. Evaluate tomorrow's demand vs current schedule.");

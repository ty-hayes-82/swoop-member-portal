  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))', gap: '16px' }}>
        ...
        <Stat label="Total Value" value={fmt$(profile.memberValueAnnual)} accent={'#E8740C'} />
        <Stat label="Account Balance" value={fmt$(profile.accountBalance)} accent={profile.accountBalance < 0 ? '#ef4444' : '#22c55e'} />
        <Stat label="Email Open Rate" value={fmtPct(profile.emailOpenRate)} accent={'#E8740C'} mono />
                ...
                <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginTop: 4 }}>
                  {(Array.isArray(preferences.favoriteSpots) ? preferences.favoriteSpots : [preferences.favoriteSpots]).map((spot, i) => (
                    <span key={i} style={{
                      ...
                      background: `${'#E8740C'}15`, border: `1px solid ${'#E8740C'}30`,
                      color: '#6B7280',
                    }}>{spot}</span>
                  ))}
                </div>
      ...
      <div style={{
        ...
        {[
          ...
          { label: 'Send SMS', icon: '📱', color: '#E8740C' },

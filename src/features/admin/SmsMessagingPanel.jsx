...
function Section({ title, children, defaultOpen = false }) {
  ...
  return (
    <div className="border border-gray-200 dark:border-gray-700 rounded-lg overflow-hidden">
      <button
        className="w-full flex items-center justify-between px-4 py-3 bg-gray-50 dark:bg-gray-800 text-sm font-semibold text-gray-700 dark:text-gray-200 cursor-pointer border-none text-left"
        onClick={() => setOpen(v => !v)}
      >
        <span>{title}</span>
...
function StatusSection({ clubId }) {
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      {/* Config form */}
      <form onSubmit={saveConfig} className="flex flex-col gap-3 pt-2 border-t border-gray-200 dark:border-gray-700">
        <p className="text-xs font-semibold text-gray-600 dark:text-gray-400 uppercase tracking-wide">Club Settings</p>
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
          <label className="flex flex-col gap-1">
            <span className="text-xs text-gray-600 dark:text-gray-400">Sender Name (shown in messages)</span>
            <input name="sender_name" defaultValue={config.sender_name} required
              className="rounded border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-900 px-2 py-1.5 text-sm text-gray-800 dark:text-gray-200" />
          </label>
          <label className="flex flex-col gap-1">
            <span className="text-xs text-gray-600 dark:text-gray-400">Max messages / member / day</span>
            <input name="max_daily_per_member" type="number" min="1" max="10" defaultValue={config.max_daily_per_member}
              className="rounded border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-900 px-2 py-1.5 text-sm text-gray-800 dark:text-gray-200" />
          </label>
          <label className="flex flex-col gap-1">
            <span className="text-xs text-gray-600 dark:text-gray-400">Quiet hours start</span>
            <input name="quiet_hours_start" type="time" defaultValue={config.quiet_hours_start}
              className="rounded border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-900 px-2 py-1.5 text-sm text-gray-800 dark:text-gray-200" />
          </label>
          <label className="flex flex-col gap-1">
            <span className="text-xs text-gray-600 dark:text-gray-400">Quiet hours end</span>
            <input name="quiet_hours_end" type="time" defaultValue={config.quiet_hours_end}
              className="rounded border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-900 px-2 py-1.5 text-sm text-gray-800 dark:text-gray-200" />
          </label>
        </div>
        <label className="flex flex-col gap-1">
          <span className="text-xs text-gray-600 dark:text-gray-400">Opt-out reply message</span>
...
function StaffSection({ clubId }) {
  ...
  if (!staff.length) return <p className="text-sm text-gray-500">No staff users found. Add team members in Club Management.</p>;
  ...
  return (
    <div className="border border-gray-200 dark:border-gray-700 rounded-lg p-3 flex flex-col gap-3">
      <div className="flex items-center gap-2">
        ...
        <span className="text-xs text-gray-400">{user.role} {user.title ? `· ${user.title}` : ''}</span>
      </div>
      <div className="flex items-center gap-2">
        <input
          value={phone}
          ...
          className="rounded border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-900 px-2 py-1 text-sm text-gray-800 dark:text-gray-200 w-40"
        />
        <button
          onClick={() => onSave(user.user_id, phone, categories)}
          disabled={saving}
          className="px-3 py-1 text-xs font-semibold rounded bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-200 hover:bg-gray-200 border border-gray-300 dark:border-gray-600 cursor-pointer disabled:opacity-50"
        >
          {saving ? 'Saving...' : 'Save'}
        </button>
        {phone && (
          <button
            ...
            className="px-3 py-1 text-xs font-semibold rounded bg-blue-50 dark:bg-blue-900/20 text-blue-600 dark:text-blue-400 hover:bg-blue-100 border border-blue-200 dark:border-blue-800 cursor-pointer"
          >
            Test
          </button>
        )}
...
function TemplatesSection({ clubId }) {
  ...
  if (!templates.length) return <p className="text-sm text-gray-500">Loading templates...</p>;

  return (
    <div className="flex flex-col gap-2">
      {templates.map(t => (
        <div key={t.template_id} className="border border-gray-200 dark:border-gray-700 rounded-lg p-3">
          <div className="flex items-center justify-between mb-2">
            <div className="flex items-center gap-2">
              <span className="text-xs font-mono font-semibold text-gray-600 dark:text-gray-400">{t.template_id}</span>
              <span className="text-xs px-1.5 py-0.5 rounded-full bg-gray-100 dark:bg-gray-700 text-gray-500">{t.category}</span>
              {t.club_id && <span className="text-xs px-1.5 py-0.5 rounded-full bg-blue-50 dark:bg-blue-900/20 text-blue-600 dark:text-blue-400">customized</span>}
            </div>
            <button
          ...
          {editing === t.template_id ? (
            <div className="flex flex-col gap-2">
              <textarea
                ...
                defaultValue={t.body}
                rows={3}
                className="w-full rounded border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-900 px-2 py-1.5 text-sm text-gray-800 dark:text-gray-200 font-mono resize-y"
              />
              <div className="flex items-center gap-2">
                <button
                  onClick={() => {
                ...
                <p className="text-xs text-gray-400">Variables: {slots(t.body).map(s => `{{${s}}}`).join(', ') || 'none'}</p>
              </div>
            </div>
          ) : (
            <div>
              <p className="text-xs text-gray-600 dark:text-gray-400 font-mono leading-relaxed">{t.body}</p>
              <div className="flex flex-wrap gap-1 mt-1">
                {slots(t.body).map(s => (
                  <span key={s} className="text-xs px-1.5 py-0.5 rounded bg-gray-100 dark:bg-gray-700 text-gray-500 font-mono">{`{{${s}}}`}</span>
                ))}
              </div>
            </div>
          )}
    ...
    <div className="flex flex-col gap-3">
      ...
      {loading ? (
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-xs">
            ...
            <tbody>
              {rows.map(row => (
                <tr key={row.log_id} className="border-b border-gray-100 dark:border-gray-800 hover:bg-gray-50 dark:hover:bg-gray-800/50">
                  ...
                  <td className="py-1.5 pr-3">
                    <span className={`px-1.5 py-0.5 rounded-full text-xs ${
                      row.direction === 'inbound' ? 'bg-purple-50 dark:bg-purple-900/20 text-purple-600 dark:text-purple-400' :
                      row.direction === 'outbound_staff' ? 'bg-orange-50 dark:bg-orange-900/20 text-orange-600 dark:text-orange-400' :
      ...
      <div className="flex gap-2">
        <button
          ...
          className="px-3 py-1 text-xs rounded border border-gray-300 dark:border-gray-600 disabled:opacity-40 cursor-pointer bg-white dark:bg-gray-800 text-gray-600 dark:text-gray-400"
        >
          Prev
        </button>
...
function ConsentSection({ clubId }) {
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <div className="flex flex-col gap-2">
        <p className="text-xs font-semibold text-gray-600 dark:text-gray-400 uppercase tracking-wide">Consent Approach</p>
        {[
          {
          ...
          <label key={opt.value} className="flex gap-3 cursor-pointer border border-gray-200 dark:border-gray-700 rounded-lg p-3 hover:bg-gray-50 dark:hover:bg-gray-800/50">
            <input
              type="radio"
              name="consent_mode"
              value={opt.value}
      ...
      {mode === 'membership_agreement' && (
        <p className="text-xs bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 text-blue-700 dark:text-blue-300 rounded p-2">
      ...
      {result && (
        <div className="text-sm text-green-600 dark:text-green-400 bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded p-3">
          {mode === 'membership_agreement'
...
export default function SmsMessagingPanel({ clubId }) {
  if (!clubId) return <p className="text-sm text-gray-500">Select a club first.</p>;

  ...
  return (
    <div style={{ padding: '16px', display: 'flex', flexDirection: 'column', gap: '12px' }}>
      ...
      {topPriority && topPriority.score < 30 && (
        ...
        >
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <div>
              <div style={{ fontSize: '11px', fontWeight: 700, color: '#991B1B', textTransform: 'uppercase', letterSpacing: '0.06em' }}>Critical — Needs Attention</div>
              <div style={{ fontSize: '16px', fontWeight: 700, color: '#0F0F0F', marginTop: '4px' }}>{topPriority.name}</div>
              <div style={{ fontSize: '13px', color: '#6B7280', marginTop: '2px' }}>
                Health {topPriority.score} · ${(topPriority.annualDues || topPriority.duesAnnual || 18000).toLocaleString()}/yr
              </div>
            </div>
            <a
              href={`tel:`}
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>
        ...
        <KpiTile label="Complaints" value={complaints || 3} sub="unresolved" color="#F59E0B" />
        <KpiTile label="Pending Actions" value={pendingAgentCount} sub="awaiting approval" color="#F3922D" onClick={() => navigateTab('inbox')} />
        <KpiTile label="Revenue" value={`↑ 10.7%`} sub="vs last week" color="#22C55E" />
      </div>
      ...
      {topAction && (
        <div style={{
          ...
          <div style={{ fontSize: '11px', fontWeight: 700, color: '#F3922D', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: '6px' }}>
            Top Action
          </div>
...
function KpiTile({ label, value, sub, color, onClick }) {
  return (
    ...
    >
      <div style={{ fontSize: '11px', color: '#6B7280', textTransform: 'uppercase', letterSpacing: '0.04em' }}>{label}</div>

    ...
    <>
      ...
      {open && (
        <div className="fixed inset-0 z-[1000] bg-black/50 flex items-center justify-center p-4 sm:p-8">
          <div className="bg-swoop-panel text-swoop-text rounded-2xl w-full max-w-[640px] max-h-[90vh] overflow-auto shadow-theme-xl">
            {/* Print header */}
            <div className="px-8 py-6 border-b-2 border-blue-light-500 bg-swoop-row">
              <div className="flex justify-between items-start">
                <div>
                  <div className="text-[22px] font-normal text-swoop-text">
                    {getClubName()}
                  </div>
                ...
                <div className="flex gap-2">
                  ...
                  <button onClick={() => setOpen(false)} className="px-3.5 py-2 rounded-md text-sm cursor-pointer border border-swoop-border bg-transparent text-swoop-text-muted">
                    Close
                  </button>
                </div>
              </div>
            ...
            <div className="px-8 py-6 flex flex-col gap-5">
              ...
              <section>
                ...
                {!collapsed.results && (
                  <div className="bg-swoop-row rounded-lg px-4 py-3.5 text-sm leading-relaxed text-swoop-text">
              ...
              <section>
                ...
                {!collapsed.actions && (
                  <div className="bg-swoop-row rounded-lg px-4 py-3">
                    {(briefing.quickWins || []).map((win, idx) => (
                      <div key={win.id} className={`pb-2.5 ${idx < briefing.quickWins.length - 1 ? 'mb-2.5 border-b border-swoop-border' : ''}`}>
                        <div className="text-sm font-semibold text-swoop-text mb-1">{win.icon} {win.title}</div>
                        <div className="text-xs text-swoop-text-muted leading-relaxed">{win.detail}</div>
              ...
              <section>
                ...
                {!collapsed.members && atRisk.filter(m => m.score < 40).map(m => (
                  <div key={m.memberId} className="border-b border-swoop-border py-2.5 flex justify-between">
                    <div>
                      <MemberLink memberId={m.memberId} className="text-sm font-semibold text-swoop-text">{m.name}</MemberLink>
                      <div className="text-xs text-swoop-text-muted mt-0.5">{m.topRisk}</div>
                    </div>
              ...
              <section>
                ...
                {!collapsed.staffing && (
                  <div className="bg-swoop-row rounded-lg px-4 py-3.5 text-sm text-swoop-text leading-relaxed">
                    ...
                    {briefing.todayRisks.forecast && (
                      <div className="mt-1 text-sm text-swoop-text-muted">{briefing.todayRisks.forecast}</div>
                    )}
                    <div className="mt-1">
              ...
              {/* Notes */}
              <section>
                <SectionHeader label="Department Head Notes" colorCls="text-swoop-text-muted" sectionKey="notes" />
                {!collapsed.notes && ['Golf Operations', 'F&B', 'Membership', 'Grounds'].map(dept => (
                  <div key={dept} className="border-b border-swoop-border py-3 flex items-center gap-3">
                    <div className="text-sm text-swoop-text-muted w-[120px] shrink-0">{dept}</div>
                    <div className="flex-1 border-b border-swoop-border h-5" />
                  </div>
                ))}
              </section>

              <div className="text-[11px] text-swoop-text-label text-center border-t border-swoop-border pt-4">

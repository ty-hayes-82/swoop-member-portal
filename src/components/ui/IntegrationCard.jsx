...
export default function IntegrationCard({
  ...
  return (
    <button
      ...
      className={`block w-full text-left rounded-xl p-4 cursor-pointer transition-all duration-150 ${
        isSelected
          ...
          : 'bg-swoop-panel border-[1.5px] border-swoop-border'
    ...
    >
      ...
      <div className="flex items-start justify-between mb-2">
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <div className="text-[10px] text-swoop-text-muted mt-px">{category}</div>
          </div>
        </div>
      ...
      {/* Description */}
      <p className="text-[11px] text-swoop-text-muted leading-relaxed m-0 mb-2">
        {description}
      </p>

      {/* Partners */}
      <div className="text-[10px] text-swoop-text-muted">
  ...
  return (
    <div className="flex flex-col items-end gap-0.5 shrink-0">
      <div className="flex items-center gap-1">
        ...
        <span className={`text-[10px] font-semibold ${cfg.dotCls === 'bg-success-500' ? 'text-success-500' : cfg.dotCls === 'bg-warning-500' ? 'text-warning-500' : 'text-swoop-text-label'}`}>{cfg.label}</span>
      </div>
      {lastSync && (
        <span className="text-[10px] text-swoop-text-muted">Synced {lastSync}</span>
      )}
    </div>
  );
}

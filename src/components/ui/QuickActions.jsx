...
// "Draft a note", "Schedule a call", "Assign to F&B Director"
// Critique Phase 3: close the gap between insight and action.
...
export default function QuickActions({ memberName, memberId, context = '' }) {
  const [mode, setMode]   = useState(null); // null | 'note' | 'call' | 'task'
  ...
  const btn = (label, icon, key, color) => (
    <button
      key={key}
      ...
      style={{
        ...
        border: `1px solid ${mode === key ? color : theme.colors.border}`,
        background: mode === key ? `${color}12` : theme.colors.bgCard,
        color: mode === key ? color : theme.colors.textSecondary,
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', gap: theme.spacing.sm, flexWrap: 'wrap' }}>
        {btn('Draft personal note', '✉', 'note', theme.colors.briefing)}
        {btn('Schedule a call', '📞', 'call', theme.colors.success)}
        {btn('Assign to staff', '→', 'task', theme.colors.staffing)}
      </div>
        ...
        <div style={{ marginTop: theme.spacing.sm, padding: theme.spacing.md,
          ...
          <textarea
            ...
            style={{ width: '100%', height: 160, padding: theme.spacing.sm, fontSize: theme.fontSize.sm,
              ...
              borderRadius: theme.radius.sm, resize: 'vertical', lineHeight: 1.6, outline: 'none' }}
          />
        ...
        <div style={{ marginTop: theme.spacing.sm, padding: theme.spacing.md,
          ...
          <div style={{ display: 'flex', gap: theme.spacing.sm }}>
            ...
            <button onClick={() => setMode(null)} style={{
              padding: '7px 14px', borderRadius: theme.radius.sm, fontSize: theme.fontSize.sm,
              cursor: 'pointer', border: `1px solid ${theme.colors.border}`,
              background: 'none', color: theme.colors.textMuted }}>Dismiss</button>
          </div>
        </div>
      )}

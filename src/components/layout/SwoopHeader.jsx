...
const SwoopHeader = () => {
  ...
  return (
    <header role="banner" className="sticky top-0 flex w-full bg-swoop-panel border-b border-swoop-border z-99999">
      <div className="flex items-center justify-between w-full gap-4 px-3 py-3 lg:px-6 lg:py-4">
        ...
        <div className="flex items-center gap-2 sm:gap-4">
          ...
          <button
            className={`flex items-center justify-center w-10 h-10 text-swoop-text-muted rounded-lg z-99999 lg:h-11 lg:w-11 lg:border lg:border-swoop-border ${
              isMobileOpen ? "bg-swoop-row" : ""
            }`}
            ...
            <form>
              <div className="relative">
                ...
                <input
                  ...
                  className="h-11 w-full rounded-lg border border-swoop-border bg-transparent py-2.5 pl-12 pr-14 text-sm text-swoop-text shadow-theme-xs placeholder:text-swoop-text-label focus:border-brand-300 focus:outline-hidden focus:ring-3 focus:ring-brand-500/10 lg:w-[430px]"
                />
                <button className="absolute right-2.5 top-1/2 inline-flex -translate-y-1/2 items-center gap-0.5 rounded-lg border border-swoop-border bg-swoop-row px-[7px] py-[4.5px] text-xs -tracking-[0.2px] text-swoop-text-muted">
                  <span>&#8984;</span>
                  <span>K</span>
                </button>
                {showSearchResults && (
                  <div className="absolute left-0 top-12 w-full rounded-xl border border-swoop-border bg-swoop-panel shadow-theme-lg z-50 max-h-80 overflow-y-auto">
                    {searchResults.length === 0 ? (
                      <div className="p-3 text-sm text-swoop-text-muted text-center">No results found</div>
                    ) : (
                      searchResults.map((r) => (
                        <button
                          key={r.id}
                          ...
                          className="w-full text-left px-4 py-2.5 hover:bg-swoop-row-hover border-b border-swoop-border-inset cursor-pointer bg-transparent border-x-0 border-t-0"
                        >
                          <div className="text-sm font-semibold text-swoop-text">{r.name}</div>
                          <div className="text-xs text-swoop-text-muted">{r.email} {r.archetype ? `\u00B7 ${r.archetype}` : ''}</div>
                        </button>
                      ))
                    )}
                  </div>
                ...
                {showCommandHint && !showSearchResults && !searchQuery && (
                  <div className="absolute left-0 top-12 w-full rounded-xl border border-swoop-border bg-swoop-panel shadow-theme-lg z-50">
                    <div className="p-3 text-xs text-swoop-text-label font-semibold uppercase tracking-wide border-b border-swoop-border-inset">Quick Actions</div>
                    {[
                      ...
                      <button
                        ...
                        className="w-full text-left px-4 py-2.5 hover:bg-swoop-row-hover border-b border-swoop-border-inset last:border-b-0 cursor-pointer bg-transparent border-x-0 border-t-0 flex items-center gap-3"
                      >
                        <span className="text-base">{cmd.icon}</span>
                        <div>
                          <div className="text-sm font-semibold text-swoop-text">{cmd.label}</div>
                          <div className="text-xs text-swoop-text-label">{cmd.hint}</div>
                        </div>
                      </button>
                    ))}
                  </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="relative" ref={notifRef}>
            <button
              ...
              className="relative flex items-center justify-center w-10 h-10 rounded-full text-swoop-text-muted hover:bg-swoop-row-hover hover:text-swoop-text-2 transition-colors"
            ...
            {notifOpen && (
              <div className="absolute right-0 top-12 w-[calc(100vw-2rem)] sm:w-80 max-h-96 overflow-y-auto rounded-xl border border-swoop-border bg-swoop-panel shadow-theme-lg z-50">
                <div className="p-3 border-b border-swoop-border font-semibold text-sm text-swoop-text">Notifications</div>
                <div className="p-3 text-sm text-swoop-text-muted text-center">No new notifications</div>
              </div>
            )}
          </div>
          ...
          <div className="relative" ref={userMenuRef}>
            <button
              ...
              className="flex items-center gap-3 cursor-pointer rounded-lg px-2 py-1.5 hover:bg-swoop-row-hover transition-colors"
            >
              <div className="text-right hidden sm:block">
                <p className="text-sm font-medium text-swoop-text m-0">
                  {userName}
                </p>
              ...
              <svg className={`w-4 h-4 text-swoop-text-label transition-transform ${userMenuOpen ? "rotate-180" : ""}`} viewBox="0 0 20 20" fill="currentColor">
            ...
            {/* Dropdown menu */}
            {userMenuOpen && (
              <div className="absolute right-0 top-full mt-2 w-56 rounded-xl border border-swoop-border bg-swoop-panel shadow-theme-lg z-50 py-1">
                {/* User summary */}
                <div className="px-4 py-3 border-b border-swoop-border-inset">
                  <p className="text-sm font-semibold text-swoop-text m-0">{userName}</p>
                  <p className="text-xs text-swoop-text-muted m-0 mt-0.5">
                ...
                <div className="py-1">
                  <button
                    ...
                    className="w-full text-left px-4 py-2 text-sm text-swoop-text-2 hover:bg-swoop-row-hover cursor-pointer flex items-center gap-2.5"
                  >
                    <svg className="w-4 h-4 text-swoop-text-label" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
                ...
                <div className="border-t border-swoop-border-inset py-1">
                  <button
                    onClick={() => {
                      setUserMenuOpen(false);
                      loadStaticDemo();
                ...
                <div className="border-t border-swoop-border-inset py-1">
                  <button
                    ...
                    className="w-full text-left px-4 py-2 text-sm text-red-600 hover:bg-red-50 cursor-pointer flex items-center gap-2.5"
                  >

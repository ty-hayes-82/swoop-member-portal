...
export default async function handler(req, res) {
  ...
  if (IS_PROD && !ALLOW_DEBUG) {
    logWarn('/api/seed-agents', 'operator endpoint blocked in production', { ip: req.headers['x-forwarded-for'] });
  ...
  try {
    ...
    await sql`INSERT INTO agent_actions (action_id, agent_id, action_type, priority, source, description, impact_metric, member_id, status, timestamp)
      VALUES
        ...
        ('agx_008', 'service-recovery', 'alert', 'medium', 'POS + Staff Notes', 'David Chen waited 22 min for starter. Pro shop noted he seemed frustrated. No recovery action logged.', 'Health score dropped to 54', 'mbr_t06', 'pending', NOW() - INTERVAL '5 hours'),
